function  [MaxClusterIndex, MaxPercIndex, MaxMaxClusterIndex, AreaRatioMaxCluster, MaxMaxCluster, SkewnessMaxCluster, AreaMaxCluster, AreaRatioCluster, AreaRatioPerc, MaxCluster, MaxPerc, SkewnessCluster, SkewnessPerc, AreaPerc, AreaCluster, p,g,h] = clustpercEucl(img,maxr)%inicia fun��o de gera��o da matriz de prob. com par�metros imagem e R Max
    aux=double(img);
    r=3:2:maxr; %iterar de r de 3 a R max de 2 em 2
    g=zeros(1,size(r,2)); %cria��o de vetor para armazenar os valores de percola��o para cada tamanho de r
    p=zeros(1,size(r,2));%cria��o de vetor para armazenar os valores de n. aglomerados para cada tamanho de r
    h=zeros(1,size(r,2)); %vetor para armazenar o valor do maior aglomerado m�dio para cada tamanho de r
    %cada tamanho de caixa
    parfor k=1:size(r,2)
        vetBigClusters=zeros(1,r(k)^2); %vetor que armazena a ocupa��o do maior cluster em cada caixa
        ptemp=0;
        gtemp=0;
        ...
        for x=lim+1:(size(img,1)-lim)         
            ...
            a=0;
               ...
               [~, F] = mode(L(L>0)); %calcula qual a frequ�ncia do maior aglomerado nesta caixa
               vetBigClusters(caixa) = F/(r(k)^2); %armazena a porcentagem que ele ocupa na caixa
                ptemp=ptemp+ROTULO;
                if(percCount/(r(k)^2)>=0.59275) %se o n�mero total de pixels percolantes ofr maior que o limiar
                    gtemp=gtemp+1;
                end                
                caixa=caixa+1;
            end
